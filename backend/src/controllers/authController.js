...
exports.signup = async (req, res, next) => {
  ...
  const user = await User.create({ name, email, password, role, image_url });
    // Reload the instance to ensure any DB-side defaults/columns (e.g. newly added columns)
    // are present on the returned object (prevents nulls when DB schema was altered).
    await user.reload();
